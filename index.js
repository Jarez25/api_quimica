...
// ** Aquí habilitamos CORS **
app.use(
  cors({
    origin: 'http://localhost:5173', // Cambia esto al puerto o dominio de tu frontend
...
app.get('/', (req, res) => {
  ...
  const endpoints = [
    ...
    {
      path: '/elementos/:id',
      description: 'Obtener elemento por ID MongoDB (reemplaza :id)',
    },
    {
      path: '/elementos/atomic_number/:atomic_number',
      description:
        'Obtener elemento por número atómico (reemplaza :atomic_number)',
    },
    {
      path: '/elementos/symbol/:symbol',
      description: 'Obtener elemento por símbolo químico (reemplaza :symbol)',
    },
    {
      path: '/elementos/name/:name',
      description: 'Obtener elemento por nombre (reemplaza :name)',
    },
    {
      path: '/elementos/grupo/:grupo',
      description:
        'Obtener elementos por grupo y nombre de familia (reemplaza :grupo)',
    },
    ...
    <ul>
      ${endpoints
        .map(
          (ep) => `
        <li>
          <a href="${baseURL}${ep.path.replace(/:.*$/, '')}" target="_blank">${
            ep.path
          }</a> - ${ep.description}
        </li>
      `
